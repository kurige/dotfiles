...
TMUX_POWERLINE_SEG_WEATHER_DATE_DEFAULT="/usr/local/bin/gdate"
...
# Name of GNU date binary if in PATH, or path to it.
export TMUX_POWERLINE_SEG_WEATHER_DATE="${TMUX_POWERLINE_SEG_WEATHER_DATE_DEFAULT}"
...
__process_settings() {
  ...
  if [ -z "$TMUX_POWERLINE_SEG_WEATHER_DATE" ]; then
    export TMUX_POWERLINE_SEG_WEATHER_DATE="${TMUX_POWERLINE_SEG_WEATHER_DATE_DEFAULT}"
  fi
...
__yahoo_weather() {
  gnudate="${TMUX_POWERLINE_SEG_WEATHER_DATE}"
  ...
  if [ -z "$degree" ]; then    
    ...
    if [ "$?" -eq "0" ]; then
      ...
      sunrise=$("$gnudate" -d "$(echo "$weather_data" | "$gnugrep" "yweather:astronomy" | sed 's/^\(.*\)sunset.*/\1/' | sed 's/^.*sunrise="\(.*m\)".*/\1/')" +%H%M)
      sunset=$("$gnudate" -d "$(echo "$weather_data" | "$gnugrep" "yweather:astronomy" | sed 's/^.*sunset="\(.*m\)".*/\1/')" +%H%M)
